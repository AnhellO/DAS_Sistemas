...
def get_personajes():
    p = []
    personajes = Personajes.select(Personajes.id, Personajes.name, Personajes.status, Personajes.species, Personajes.type, Personajes.gender, Personajes.OriginName, Personajes.OriginUrl, Personajes.LocationName, Personajes.LocationUrl, Personajes.image,Personajes.episodes, Personajes.url, Personajes.created)
    for i in personajes:
        p.append(i)
    return p
...
def get_locaciones():
    l = []
    locaciones = Locaciones.select(Locaciones.id, Locaciones.name, Locaciones.tipe, Locaciones.dimension, Locaciones.residents, Locaciones.url, Locaciones.created)
    for i in Locaciones:
        l.append(i)
    return l
...
def get_episodios():
    e = []
    episodios = Episodios.select(Episodios.id, Episodios.name, Episodios.air_date,Episodios.episode, Episodios.characters, Episodios.url, Episodios.created)
    for i in episodios:
        e.append(i)
    return e
    ...
    return f'''<!DOCTYPE html>
        <html lang="en">
            <head>
                ...
                <div align="center"><img src="static/RyM.jpg"></div>
            </head>
            <body>
...
def index_LocacionesSp(name):
    query = get_locaciones()
    for i in query:
        if i.name == name:
            return f'''<!DOCTYPE html>
                       <html lang="en">
