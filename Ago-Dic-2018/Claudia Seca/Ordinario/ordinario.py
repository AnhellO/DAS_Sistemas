...
class Cliente():
    ...
    def __init__(self, id, first, last, city, cellphone, email, age, gender, db):
    ...
    def ordenar(self):
        ...
        print('{} ha ordenado {} tacos de {}'.format(self.first, cantidad, nombre))
        return cantidad
...
class InterfaceApiCliente(ABC):

    @abstractmethod
    def get_cliente():
        pass

class InterfaceApiTaco(ABC):

    @abstractmethod
    def get_info():
        pass

class InterfaceBD(ABC):
...
class ApiCliente(InterfaceApiCliente):
...
class TacoApi(InterfaceApiTaco):
...
class Sqlite(InterfaceBD):
    ...
    def get_cliente(self):
        ...
        objeto = Cliente(cliente[0], cliente[1], cliente[2], cliente[3], cliente[4], cliente[5], cliente[6], cliente[7], self.dbname)
        return objeto
    ...
    def insert_taco(self,info):
        ...
        cursor.execute('''UPDATE Tacos SET Precio = :precio WHERE Nombre = :nombre''',{'precio': 20, 'nombre': info[0]})
        db.commit()
