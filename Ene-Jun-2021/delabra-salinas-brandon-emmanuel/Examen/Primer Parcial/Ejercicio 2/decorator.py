#funcion para decorador de acuerdo al formato dado
def Decorator(self):
    if get_formato == 'HTML':
        print (f'<p style="color:red;text-align:center;">{self.contenido}</p' )
    elif get_formato == 'XML':
        print ('No conozco el formato'  '<lki>')
    elif get_formato == 'JSON':
        print ('No conozco el formato' '<juikl>')
    return Decorator
...
#Aplicamos una función de busqueda para verificar que la página si existe
def buscador(web:SitioWeb , pagina:PaginaWeb):
    if pagina.paginas() in web.contenido():
        print('Si existe')
        print (web)
    else:
        print ('  No existe ')
